...
exp_name=fdcl_g4_imperfect_gcdro_seed${split}
...
python -u train.py ${DATA} \
    --reweight 0 --seed ${split} --num-train-groups 4 --num-test-groups 16 \
    --skip-invalid-size-inputs-valid-test \
    --label-path ${DATA}/train.resplit.labels \
    --criterion cross_entropy_group_dro \
    --dro-alpha 0.5 --beta-cover-instances 0.25 --ema 0.1 --beta-ema 0.5 \
...
python -u test.py ${DATA} \
    --reweight 0 --seed ${split} --num-train-groups 4 --num-test-groups 14 \
    --skip-invalid-size-inputs-valid-test \
    --label-path ${DATA}/train.resplit.labels \
    --criterion cross_entropy_group_dro \
    --dro-alpha 0.2 --beta-cover-instances 1.0 --ema 0.5 \
    --resplit-greedy 1 --resplit-epoch 2 --conservative 1 \

...
def add_dataset_args(parser, train=False, gen=False):
    ...
    if train:
        ...
        group.add_argument('--test-subset', default=None, type=str)
...
def add_optimization_args(parser):
    ...
    # training options for DRO-related optimization datasets with labels
    group.add_argument('--label-path', type=str, default=None, help="path to group labels for each training instance")
    group.add_argument('--reweight', type=int, default=0, help="set to 1 to turn on resampling")
    group.add_argument('--dynamic-group-dro', type=int, default=0, help="turn this on to use GC DRO")

    # the following three arguments controls when to perform reweighting on group instances
    group.add_argument('--conservative', type=int, default=1, help="more groups, conservative")
    group.add_argument('--resplit-patience', type=int, default=-1)
    group.add_argument('--resplit-epoch', type=int, default=5,
                       help="resplit actually means re-computes the instance-level weights of each group, "
                            "it's a bad naming due to historical reasons")

    group.add_argument('--resplit-greedy', type=int, default=1,
                       help='whether do sampling when recreating group dataset for an epoch, '
                            'if use online greedy group DRO, set to 1; otherwise (Exponentiated Gradient Ascent), set to 0')
    group.add_argument('--num-train-groups', type=int, help="number of groups (clean or imperfect) at training time")
    group.add_argument('--num-test-groups', type=int, help="number of groups (clean) at test time")

    # instance reweight
    group.add_argument('--beta-cover-instances', type=float, default=0,
                       help="used in this file: fairseq/data/instance_reweight_dataset.py")

    group.add_argument('--ema', type=float, default=0.5, help="ema \lambda for outer DRO historical loss, "
                                                              "the larger, ")
    group.add_argument('--beta-ema', type=float, default=-1,
                       help="ema \lambda for inner instance DRO, if -1 use the same as the outer ema")
    group.add_argument('--log-internal', type=int, default=0, help="output some analysis logs")
    # fmt: on
    return group
